...
#define MOCPP_DF_COMPACT 0x01   //print module by file name and line number
...
#elif MOCPP_DBG_FORMAT == MOCPP_DF_COMPACT
#define MOCPP_DBG(level, X)   \
    do {                        \
        ...
        MOCPP_CONSOLE_PRINTF("%.*s:%i ", (int) (_mo_r - _mo_l), _mo_file + _mo_l,__LINE__);           \
        MOCPP_CONSOLE_PRINTF X;  \
        MOCPP_CONSOLE_PRINTF("\n");         \
    } while (0);
