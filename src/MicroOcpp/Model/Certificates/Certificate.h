...
/*
 * Print the issuerNameHash of mo_cert_hash as hex-encoded string (e.g. "0123AB") into buf. Bufsize MO_CERT_HASH_ISSUER_NAME_KEY_SIZE is always enough
 *
...
/*
 * Print the issuerKeyHash of mo_cert_hash as hex-encoded string (e.g. "0123AB") into buf. Bufsize MO_CERT_HASH_ISSUER_NAME_KEY_SIZE is always enough
 *
 * Returns the length not counting the terminating 0 on success, -1 on failure
 */
...
/*
 * Print the serialNumber of mo_cert_hash as hex-encoded string without leading 0s (e.g. "123AB") into buf. Bufsize MO_CERT_HASH_SERIAL_NUMBER_SIZE is always enough
 *
 * Returns the length not counting the terminating 0 on success, -1 on failure
 */
...
using CertificateHash = mo_cert_hash;
