...
ReservationService::ReservationService(Context& context, unsigned int numConnectors) : AllocOverrider("v16.Reservation.ReservationService"), context(context), maxReservations((int) numConnectors - 1), reservations(makeMemVector<std::unique_ptr<Reservation>>(getMemoryTag())) {
