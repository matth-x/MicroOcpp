...
TEST_CASE( "Charging sessions" ) {
    ...
    SECTION("Check idle state"){
        ...
        checkMsg.registerOperation("BootNotification", [engine] () -> Operation* {return new Ocpp16::BootNotification(engine->getModel(), std::unique_ptr<MemJsonDoc>(new MemJsonDoc(0)));});
    ...
    SECTION("Preboot transactions - reject tx if limit exceeded") {
        ...
        getOcppContext()->getOperationRegistry().registerOperation("StartTransaction", [&txId_generate] () {
            return new Ocpp16::CustomOperation("StartTransaction",
                ...
                [&txId_generate] () {
                    //create conf
                    auto doc = std::unique_ptr<MemJsonDoc>(new MemJsonDoc(JSON_OBJECT_SIZE(1) + JSON_OBJECT_SIZE(2)));
